    ...
    {#each blocks as block, i}
      <div class="group relative flex flex-col justify-end overflow-hidden transition-all duration-700">
        ...
        <div class="absolute inset-0 z-0">
          ...
          <div class="absolute inset-0 bg-linear-to-t from-black/80 via-transparent to-transparent opacity-90"></div>
        </div>
        ...
        <div class="relative z-10 flex h-full flex-col justify-end p-16 md:p-12">
          ...
          <h3 class="mb-2 text-2xl font-black uppercase tracking-tighter text-white md:text-3xl">
            {block.title}
          </h3>

          <!-- Скрытое описание, которое появляется при наведении -->
          <div class="max-h-0 overflow-hidden opacity-0 transition-all duration-500 group-hover:max-h-32 group-hover:opacity-100">
          ...
          <div class="mt-6 h-1 w-12 bg-green-500 transition-all duration-500 group-hover:w-full"></div>
        </div>

        <!-- Тонкая рамка-разделитель (опционально, для эффекта стекла) -->
        <div class="pointer-events-none absolute inset-0 border-r border-b border-white/10 last:border-0"></div>
      </div>
    {/each}
  </div>
</section>
...
<style>
  ...
  @media (min-width: 1024px) {
    .grid > div:nth-child(2n) { border-right: 1px solid rgba(255, 255, 255, 0.1); }
