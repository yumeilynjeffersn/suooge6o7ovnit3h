...
<section id="gallery" class="section bg-[var(--clr-surface)]" aria-labelledby="gallery-heading">
	...
	<div
		class="gallery-strip flex gap-4 overflow-x-auto px-[clamp(1rem,4vw,calc((100vw-var(--max-w))/2+2.5rem))] pb-3"
		role="list"
		aria-label={m.gallery_aria()}
	>
		{#each gallery as item, i}
			<figure
				class="gallery-item reveal relative m-0 aspect-[4/3] flex-[0_0_clamp(260px,35vw,480px)] cursor-pointer overflow-hidden rounded-[var(--radius-lg)]"
				style="--delay:{i * 60}ms"
				role="listitem"
			>
				<img
				...
				/>
				<figcaption
					class="gallery-caption absolute inset-x-0 bottom-0 bg-gradient-to-t from-[rgba(14,15,12,0.85)] to-transparent px-4 pt-6 pb-3 font-[family-name:var(--font-display)] text-[0.7rem] font-bold tracking-[0.1em] text-[var(--clr-text)] uppercase opacity-0"
				>
					{item.label}
				</figcaption>
			</figure>
